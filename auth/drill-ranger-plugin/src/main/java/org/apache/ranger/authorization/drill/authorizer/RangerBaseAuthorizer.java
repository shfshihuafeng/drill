...
public class RangerBaseAuthorizer {
  ...
  /**
   * Releases the underlying {@link RangerBasePlugin} resources (policy-refresh
   * threads, policy-engine caches) and clears the plugin reference.
   *
   * <p>Idempotent and thread-safe ({@code synchronized}, mutually exclusive
   * with {@link #init(String)}): calling {@code cleanUp()} without a prior
   * {@code init()}, or twice in a row, is a safe no-op. After this method the
   * instance is back to its pre-init state, so a subsequent {@code init()}
   * re-creates a fresh plugin — this matters when a Drillbit is shut down and
   * restarted inside the same JVM.</p>
   */
  public synchronized void cleanUp() {
    if (plugin == null) {
      return;
    }
    logger.info("Cleaning up RangerPlugin");
    try {
      plugin.cleanup();
    } catch (Exception e) {
      logger.warn("Error while cleaning up RangerPlugin", e);
    } finally {
      // Always drop the reference: even if cleanup() partially failed, a
      // fresh plugin must be creatable on the next init().
      plugin = null;
    }
  }
}
